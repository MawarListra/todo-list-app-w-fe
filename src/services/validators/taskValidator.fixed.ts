...
export class TaskValidator {
  ...
  static validateTitle(title: string): boolean {
    const schema = Joi.string().min(1).max(200).trim().required();
  ...
  static validateDescription(description?: string): boolean {
    ...
    const schema = Joi.string().max(1000).trim().optional();
  ...
  static validatePriority(priority?: TaskPriority): boolean {
    ...
    const schema = Joi.string().valid('low', 'medium', 'high').optional();
  ...
  static validateDeadline(deadline?: string | null): boolean {
    ...
    const schema = Joi.string().isoDate().optional();
  ...
  static validateSortCriteria(sortBy?: string, order?: string): boolean {
    ...
    const orderSchema = Joi.string().valid('asc', 'desc').optional();
